...
// NewWorkspaceStore creates a workspace store for the supplied discovery directories.
func NewWorkspaceStore(directories []string) WorkspaceStore {
	return WorkspaceStore{state: &workspaceStoreState{directories: cloneDirectories(directories)}}
}

// IDs lists the unique workspace IDs in configured discovery directories.
func (s WorkspaceStore) IDs() ([]string, error) {
...
func (s WorkspaceStore) Resolve(workspaceID string) (string, bool, error) {
	workspaces, err := s.discover()
	if err != nil {
		return "", false, err
	}
...
func (s WorkspaceStore) Reload(directories []string) {
	if s.state == nil {
		return
	}
...
func (s WorkspaceStore) discover() ([]discoveredWorkspace, error) {
