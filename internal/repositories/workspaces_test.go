...
func TestWorkspaceStoreIDsReturnsUniqueSortedWorkspaceIDs(t *testing.T) {
	...
	store := NewWorkspaceStore([]string{firstDirectory, secondDirectory})
...
func TestWorkspaceStoreIDsDiscoversOnlyDirectChildDirectories(t *testing.T) {
	...
	store := NewWorkspaceStore([]string{workspaceDirectory})
...
func TestWorkspaceStoreIDsSkipsMissingWorkspaceDirectories(t *testing.T) {
	workspaceDirectory := t.TempDir()
	missingDirectory := filepath.Join(t.TempDir(), "missing")
	mustMkdir(t, filepath.Join(workspaceDirectory, "alpha"))

	store := NewWorkspaceStore([]string{missingDirectory, workspaceDirectory})
...
func TestWorkspaceStoreResolveReturnsFirstMatchingWorkspaceDirectory(t *testing.T) {
	firstDirectory := t.TempDir()
	secondDirectory := t.TempDir()
...
func TestWorkspaceStoreUsesCanonicalPathsForIdentity(t *testing.T) {
	firstDirectory := t.TempDir()
	secondDirectory := t.TempDir()
...
func TestWorkspaceStoreIDsForDirectoriesReturnsUniqueSortedWorkspaceIDs(t *testing.T) {
	workspaceDirectory := t.TempDir()
...
func TestWorkspaceStoreReloadReplacesWorkspaceDirectories(t *testing.T) {
	firstDirectory := t.TempDir()
	secondDirectory := t.TempDir()
	mustMkdir(t, filepath.Join(firstDirectory, "first"))
	mustMkdir(t, filepath.Join(secondDirectory, "second"))

	store := NewWorkspaceStore([]string{firstDirectory})
