...
func (m *Service) Configure(shell string, agents []Agent) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Service) CloseSessionsForDirectory(directory string) int {
	m.mu.Lock()
...
func (m *Service) ActiveDirectories() []string {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Service) remove(key string, session *ProjectSession) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Service) agentKeyName(terminalName string, agentName string) string {
	if terminalName != agentTerminalName {
		return ""
	}
...
func (m *Service) defaultAgentName() string {
	for _, agent := range m.agents {
		if agent.Default {
			return agent.Name
		}
